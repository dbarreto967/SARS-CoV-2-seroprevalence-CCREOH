...
# Adjusted odd ratios of the final model "m1"
...
glmm1_model <- ImpactsOfCOVID19_clean |>
...
# Validation of the final model "m1" with interaction term

## Quality of the final model: GOF
## load package
library(blorr)
blr_test_hosmer_lemeshow(m1)
...
pred_outcomem1 <- ifelse(pred_m1 < 0.5,
                         0,
                         1)

## transform predictions into factor and set labels
pred_outcomem1 <- factor(pred_outcomem1,
                         levels = c(0, 1),
                         labels = c("Seronegative", "Seropositive"))

## compare observed vs. predicted outcome
tab_m1 <- table(ImpactsOfCOVID19_clean$sc_s1results, pred_outcomem1,
...
accuracy_m1 <- sum(diag(tab_m1)) / sum(tab_m1)
accuracy_m1
...
sensitivity_m1 <- tab_m1[2, 2] / (tab_m1[2, 2] + tab_m1[2, 1])
sensitivity_m1

## specificity
## specificity=True negatives/ (true negatives + false positives)*100
specificity_m1 <- tab_m1[1, 1] / (tab_m1[1, 1] + tab_m1[1, 2])
specificity_m1
...
## save roc object
res_m1 <- roc(sc_s1results ~ fitted(m1),
              data = ImpactsOfCOVID19_clean)

## plot ROC curve
ggroc(res_m1, legacy.axes = TRUE)
...
ggroc(res_m1, legacy.axes = TRUE) +
  labs(title = paste0("AUC = ", round(res_m1$auc, 2)))
